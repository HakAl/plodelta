...
using PlodeltaImport.Reporting;
...
public class HoldemManagerInstaller
{
    ...
    public enum InstallerErrorCode
    {
        ...
        private async Task WithRetryAsync(Func<Task> operation, int maxRetries = 3, int delayMs = 1000,
        ...
        private async Task<T> WithRetryAsync<T>(Func<Task<T>> operation, int maxRetries = 3, int delayMs = 1000,
            CancellationToken cancellationToken = default)
        {
            for (int i = 0; i < maxRetries; i++)
            {
            ...
            foreach (var registryPath in registryPaths)
            {
                ...
                if (key != null)
                {
                    foreach (string subKeyName in key.GetSubKeyNames())
                    {
                        ...
                        if (subKey?.GetValue("DisplayName")?.ToString()?.Contains(InstallerConstants.ProductName) ==
                            true)
                        {
        ...
        private async Task<string> DownloadInstallerAsync(Uri exeUrl, CancellationToken cancellationToken = default)
        {
            ...
            if (!string.IsNullOrEmpty(downloadContentType) && downloadContentType.Contains("text/html"))
            {
                ...
                string errorBody = ExtractHtmlBody(errorHtml) ??
                                   (errorHtml.Length > 200 ? errorHtml.Substring(0, 200) + "..." : errorHtml);
        ...
        private void Handle16BitInstaller(string filePath)
        {
            ...
            string message =
                $"The downloaded installer is a 16-bit application which is not supported on modern Windows.\n\n" +
        ...
        private async Task RunInstallerAsync(string filePath, bool isValidExe, bool isValidMsi,
            CancellationToken cancellationToken = default)
        {
            ...
            else if (isValidMsi)
            {
                psi = new ProcessStartInfo
                {
                    FileName = "msiexec.exe",
                    Arguments =
                        $@"/i ""{filePath}"" /quiet /norestart TARGETDIR=""{_config.InstallPath}"" /l*vx ""{logFile}""",
                    UseShellExecute = true,
                    CreateNoWindow = false,
                    WorkingDirectory = Path.GetTempPath(),
                    Verb = "runas"
        ...
        private static string? ExtractDownloadUrl(string html)
        {
            ...
            var patterns = new[]
            {
                (Regex: new Regex(@"<meta\s+http-equiv\s*=\s*[""']refresh[""']\s+content\s*=\s*[""']\d+;\s*url\s*=\s*([^""']+)", RegexOptions.IgnoreCase),
                    GroupIndex: 1),
                (Regex: new Regex(@"window\.location\s*=\s*[""']([^""']+)[""']", RegexOptions.IgnoreCase),
                    GroupIndex: 1),
                (Regex: new Regex(@"<a\s+[^>]*href\s*=\s*[""']([^""']*\.(?:exe|msi))[""'][^>]*>", RegexOptions.IgnoreCase),
                    GroupIndex: 1)
            };
        ...
        public static string? FindExeLink(string html)
        {
            if (string.IsNullOrEmpty(html)) return null;
